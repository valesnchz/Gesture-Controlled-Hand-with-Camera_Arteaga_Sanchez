...
void setup() {
  ...
  // 3. Conexión WiFi a "UITEY EDUCA" (Red Abierta)
  Serial.print("[WIFI] Conectando a la red '");
  Serial.print(ssid);
  Serial.println("'...");
  
  WiFi.begin(ssid, NULL); // NULL significa que no requiere contraseña
  int reintentos = 0;
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.print("[WIFI] Direccion IP asignada: ");
    Serial.println(WiFi.localIP());
  } else {
    Serial.println("\n[ERROR] No se pudo conectar a la red institucional.");
    Serial.println("        Iniciando modo de respaldo: Creando red 'CamaraESP32'...");
    WiFi.softAP("CamaraESP32", "");
    Serial.print("[WIFI] IP de Respaldo: ");
